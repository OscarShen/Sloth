...
namespace sloth { 
	...
	void WaterRenderer::render(std::vector<WaterTile>& water, RawCamera & camera, const Light &light)
	{
		...
		for (auto it = water.begin(); it != water.end(); ++it) {
			glm::mat4 model = Maths::createModelMatrix(glm::vec3(it->getCenterX(), it->getHeight(), it->getCenterZ()), 0.0f, 0.0f, 0.0f, WATER_TILE_SIZE);
	...
	void WaterRenderer::prepareRender(const RawCamera &camera, const Light &light)
	{
		...
		m_MoveFactor += WATER_WAVE_SPEED * static_cast<float>(Timer::deltaFrameTime);
