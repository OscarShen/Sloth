...
using namespace std;
namespace sloth {
		sloth::MetaFile::MetaFile(const std::string & file_path)
		{
		...
		void sloth::MetaFile::openFile(const std::string & file_path)
		{
		...
		void sloth::MetaFile::loadPaddingData()
		{
			processNextLine();
		...
		bool sloth::MetaFile::processNextLine()
		{
			m_Values.clear();
			std::string line;
			...
			split(line, std::string(META_FILE_SPLITTER), &splits);
			for (size_t i = 0; i < splits.size(); ++i) {
				std::vector<std::string> parts;
				split(splits[i], std::string("="), &parts);
				if (parts.size() == 2) {
		...
		std::shared_ptr<std::vector<int>> sloth::MetaFile::getValuesOfVariable(std::string & variable)
		{
			std::shared_ptr<std::vector<int>> values = std::make_shared<std::vector<int>>();
			auto it = m_Values.find(variable);
			if (it != m_Values.end()) {
				std::vector<std::string> s;
				split(it->second, std::string(META_FILE_NUMBER_SEPARATOR), &s);
		...
		int sloth::MetaFile::getValueOfVariable(std::string & variable)
		{
			auto it = m_Values.find(variable);
			if (it != m_Values.end()) {
		...
		void sloth::MetaFile::loadLineSizes()
		{
			processNextLine();
		...
		void sloth::MetaFile::loadCharacterData(int imageWidth)
		{
			// Ìø¹ýÁ½ÐÐ
			processNextLine();
			processNextLine();
		...
		std::shared_ptr<Character> sloth::MetaFile::loadCharacter(int imageSize)
		{
